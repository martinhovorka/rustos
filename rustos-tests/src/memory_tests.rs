...
fn test_heap_alloc() {
    ...
    assert_test!(
        HEAP.alloc_count.load(Ordering::SeqCst) == initial_allocs + 1,
        "Alloc count should increment"
    );
...
impl MockMemoryPool {
    ...
    fn reset(&self) {
        self.free_bitmap
            .store((1 << POOL_BLOCK_COUNT) - 1, Ordering::SeqCst);
        self.alloc_count.store(0, Ordering::SeqCst);
    }
}
...
fn test_pool_alloc() {
    ...
    assert_test!(
        initial == POOL_BLOCK_COUNT,
        format!("Should have {} free blocks", POOL_BLOCK_COUNT)
    );

    let block = POOL.alloc();
    assert_test!(block.is_some(), "Allocation should succeed");
    assert_test!(
        POOL.available_blocks() == initial - 1,
        "Should have one less free block"
    );

    POOL.free(block.unwrap());
    assert_test!(
        POOL.available_blocks() == initial,
...
fn test_pool_exhaustion() {
    ...
    assert_test!(
        POOL.available_blocks() == POOL_BLOCK_COUNT,
...
fn test_stack_guard_init() {
    ...
    assert_test!(
        stack.usable_size() == STACK_SIZE - 16,
        "Usable size should exclude guard"
    );
}
...
fn test_memory_region_attributes() {
    ...
    assert_test!(
        !periph.is_cacheable(),
        "Peripherals should not be cacheable"
    );
}
...
fn test_alignment_power_of_two() {
    ...
    for align in [1usize, 2, 4, 8, 16, 32, 64, 128, 256] {
        assert_test!(
            ...
            format!("{} should be power of two", align)
        );
    }
}
...
fn test_concurrent_pool_alloc() {
    ...
    assert_test!(
        alloc_count.load(Ordering::SeqCst) > 0,
        "Some pool allocations should succeed"
    );
    assert_test!(
        POOL.available_blocks() == POOL_BLOCK_COUNT,
        "All blocks should be free"
