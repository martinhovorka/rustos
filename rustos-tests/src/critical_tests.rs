...
fn test_critical_section_basic() {
    ...
    assert_test!(interrupts_enabled(), "Interrupts should be enabled initially");
    assert_test!(get_nesting_level() == 0, "Nesting should be 0 initially");
    
    let mie = enter_critical();
    
    assert_test!(!interrupts_enabled(), "Interrupts should be disabled in critical section");
    assert_test!(get_nesting_level() == 1, "Nesting should be 1 after enter");
    
    exit_critical(mie);
    
    assert_test!(interrupts_enabled(), "Interrupts should be restored after exit");
...
fn test_critical_section_nesting() {
    ...
    // Exit level 3
    exit_critical(mie3);
    assert_test!(get_nesting_level() == 2, "Nesting should be 2 after first exit");
    assert_test!(!interrupts_enabled(), "Interrupts should still be disabled");
    
    // Exit level 2
    exit_critical(mie2);
    assert_test!(get_nesting_level() == 1, "Nesting should be 1 after second exit");
    assert_test!(!interrupts_enabled(), "Interrupts should still be disabled");
    
    // Exit level 1
    exit_critical(mie1);
    assert_test!(get_nesting_level() == 0, "Nesting should be 0 after final exit");
    assert_test!(interrupts_enabled(), "Interrupts should be restored after all exits");
}
...
fn test_critical_section_raii_guard() {
    ...
    
    {
        ...
        assert_test!(!interrupts_enabled(), "Interrupts should be disabled with guard");
        assert_test!(get_nesting_level() == 1, "Nesting should be 1 with guard");
    }
    
    assert_test!(interrupts_enabled(), "Interrupts should be restored after guard drops");
    assert_test!(get_nesting_level() == 0, "Nesting should be 0 after guard drops");
}
...
fn test_critical_section_raii_nested() {
    ...
    
    {
        ...
        
        {
            ...
            assert_test!(get_nesting_level() == 2, "Nesting should be 2 after inner drop");
        }
        
        assert_test!(get_nesting_level() == 1, "Nesting should be 1 after middle drop");
    }
    
    assert_test!(get_nesting_level() == 0, "Nesting should be 0 after all drops");
...
fn test_critical_section_preserves_previous_state() {
    ...
    let mie = enter_critical();
    assert_test!(mie != 0, "Previous state should indicate interrupts were enabled");
    ...
    let mie2 = enter_critical();
    assert_test!(mie2 == 0, "Previous state should indicate interrupts were disabled");
...
fn test_critical_section_atomic_operations() {
    ...
    assert_test!(
        ...
        format!("Counter should be 1000, got {}", counter.load(Ordering::SeqCst))
    );
}
...
fn test_critical_section_guard_early_return() {
    ...
    assert_test!(get_nesting_level() == 0, "Nesting should be 0 after early return");
    assert_test!(interrupts_enabled(), "Interrupts should be enabled after early return");
    
    let result = operation_with_early_return(false);
    assert_test!(!result, "Should return false");
    assert_test!(get_nesting_level() == 0, "Nesting should be 0 after normal return");
}
